...
spline_plot <- ggplot(data=contrast_spline_df, aes(x=qdiabeteshf_5yr_score, y=exp(Contrast))) +
  ...
  xlab(expression(paste("5-year absolute heart failure risk (%)"))) +
  ylab("Heart failure HR for SGLT2i vs control") +
  coord_trans(y = "log10") +
  scale_x_continuous(breaks = seq(0,15,2.5)) +
...
spline_plot <- spline_plot +
  annotate(geom="text", x=3, y=2.4, label="Overall HR in study cohort (95% CI): 0.70 (0.63-0.78)", color="black", hjust=0, size = 6) +
  annotate(geom="text", x=3, y=2.2, label="p-value for HF risk * drug arm interaction on HF outcome: 0.71", color="black", hjust=0, size = 6) +
  annotate(geom="text", x=3, y=1.9, label="Trial meta-analysis HR (95% CI): 0.63 (0.50-0.80)", color="red", hjust=0, size = 6) 
...
tiff("HR_by_HF_risk.tiff", width=9, height=8, units = "in", res=800) 
...
contrast_spline <- contrast(model_3, list(studydrug = "SGLT2", qdiabeteshf_5yr_score = seq(c1, c99, by=0.05)), list(studydrug = "DPP4SU", qdiabeteshf_5yr_score = seq(c1, c99, by=0.05)))

contrast_spline_df <- as.data.frame(contrast_spline[c('qdiabeteshf_5yr_score','Contrast','Lower','Upper')])


setwd("/slade/CPRD_data/Katie SGLT2/Processed data/Plots/")
tiff("HR_by_HF_risk_3_knots.tiff", width=9, height=6, units = "in", res=800) 

ggplot(data=contrast_spline_df, aes(x=qdiabeteshf_5yr_score, y=exp(Contrast))) +
  geom_line(data=contrast_spline_df,aes(x=qdiabeteshf_5yr_score, y=exp(Contrast)), size=1) +
  xlab(expression(paste("5-year absolute heart failure risk (%)"))) +
  ylab("Heart failure HR for SGLT2i vs control") +
  coord_trans(y = "log10") +
...
contrast_spline <- contrast(model_4, list(studydrug = "SGLT2", qdiabeteshf_5yr_score = seq(c1, c99, by=0.05)), list(studydrug = "DPP4SU", qdiabeteshf_5yr_score = seq(c1, c99, by=0.05)))

contrast_spline_df <- as.data.frame(contrast_spline[c('qdiabeteshf_5yr_score','Contrast','Lower','Upper')])


setwd("/slade/CPRD_data/Katie SGLT2/Processed data/Plots/")
tiff("HR_by_HF_risk_4_knots.tiff", width=9, height=6, units = "in", res=800) 
